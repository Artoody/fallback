...
2. اگه کد پاسخ `429` (Rate Limit) یا `500`/`503` (خطای سرور گوگل) باشه → اون کلید به مدت `KEY_COOLDOWN_MS` (پیش‌فرض ۶۰ ثانیه) کنار گذاشته میشه و کلید بعدی امتحان میشه.
3. اگه خطای دیگه‌ای باشه (مثلاً `400` - درخواست نامعتبر) → همون خطا مستقیم به کلاینت برمی‌گرده، چون تعویض کلید کمکی نمی‌کنه.
4. اگه همه کلیدها fail بشن → خطای نهایی با `allKeysExhausted: true` برمی‌گرده.
